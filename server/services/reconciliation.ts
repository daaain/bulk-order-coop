...
export function validateDeliveryUpdate(body: unknown): DeliveryUpdateInput | { error: string } {
  ...
  if (status === 'different_price') {
    if (typeof actualPrice !== 'number' || actualPrice <= 0) {
      return {
        error: 'actualPrice is required and must be positive when status is different_price',
      };
    }
  }
