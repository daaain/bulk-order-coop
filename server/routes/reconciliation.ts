...
app.get('/:id/reconciliation', async (c) => {
	...
	for (const reconItem of reconItems) {
		...
		for (const alloc of reconItem.allocations) {
			...
			const cost = estimateCost(alloc.amount, caseSize, reconItem.delivery?.status === 'different_price' && reconItem.delivery.actualPrice ? reconItem.delivery.actualPrice : ci.casePrice, ci.vatRate);
