...
switch (command) {
  ...
  case 'seed-member': {
    ...
    db.run('INSERT INTO members (id, email, name, initials, created_at) VALUES (?, ?, ?, ?, ?)', [
      id,
      email,
      name,
      initials,
  ...
  case 'count-claims': {
    const rows = db
      .query(
        'SELECT c.id, c.member_id, c.amount, oi.order_id FROM claims c JOIN order_items oi ON c.order_item_id = oi.id',
      )
      .all();
