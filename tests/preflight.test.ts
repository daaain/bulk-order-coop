...
/** Fingerprint one table the way scripts/migrate.ts reads it. */
function table(
  name: string,
...
const orders = (rows: Record<string, unknown>[]) => table('orders', rows);
...
describe('compareData', () => {
  ...
  it('flags lost rows', () => {
    ...
    expect(compareData(before, after, none).problems).toContain(
      'Table orders lost 1 of its 2 row(s), e.g. (id=b)',
    );
  });
  ...
  it('flags a row that was replaced by another, though the count is unchanged', () => {
    const after = orders([
      { id: 'a', status: 'open', total: 10 },
      { id: 'c', status: 'open', total: null },
    ]);
    expect(compareData(before, after, none).problems).toEqual([
      'Table orders lost 1 of its 2 row(s), e.g. (id=b)',
    ]);
  ...
  it('matches rows on a composite key', () => {
    const members = (rows: Record<string, unknown>[]) =>
      table('order_members', rows, ['order_id', 'member_id']);
    const was = members([
      { order_id: 'o1', member_id: 'm1', role: 'organiser' },
      { order_id: 'o1', member_id: 'm2', role: 'member' },
    ]);
    const swapped = members([
      { order_id: 'o1', member_id: 'm1', role: 'organiser' },
      { order_id: 'o2', member_id: 'm2', role: 'member' },
    ]);
    expect(compareData(was, swapped, none).problems).toEqual([
      'Table order_members lost 1 of its 2 row(s), e.g. (order_id=o1, member_id=m2)',
    ]);
  });

  it('falls back to row counts for a table without a primary key', () => {
    const log = (rows: Record<string, unknown>[]) => table('log', rows, null);
    const was = log([{ msg: 'a' }, { msg: 'b' }]);

    const replaced = compareData(was, log([{ msg: 'a' }, { msg: 'c' }]), none);
    expect(replaced.problems).toEqual([]);
    expect(replaced.notes).toContain(
      'Table log has no primary key, so only row counts were compared',
    );

    expect(compareData(was, log([{ msg: 'a' }]), none).problems).toEqual([
      'Table log lost 1 of its 2 row(s)',
    ]);
  });

  it('falls back to row counts when a key column is dropped', () => {
    const after = table('orders', [{ status: 'open' }, { status: 'open' }], ['id']);
    after.orders.keys = null;
    const result = compareData(before, after, new Set(['orders.id', 'orders.total']));
    expect(result.problems).toEqual([]);
    expect(result.notes).toContain(
      'Table orders: its primary key (id) no longer exists, so only row counts were compared',
    );
  });
  ...
  it('lets through only the losses a migration names', () => {
    ...
    expect(result.problems).toEqual(['Table orders lost 1 of its 2 row(s), e.g. (id=b)']);
    expect(result.notes).toContain(
