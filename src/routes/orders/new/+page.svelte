<script lang="ts">
  ...
  function nextThursdaySecondWeek(): string {
    const d = new Date(now);
    // Move to next Thursday (day 4)
    const daysUntil = (4 - d.getDay() + 7) % 7 || 7;
    d.setDate(d.getDate() + daysUntil + 7); // +7 for second week
    return d.toISOString().slice(0, 10);
  }

  let deadline = $state(nextThursdaySecondWeek());

  const dayOfWeekFormatter = new Intl.DateTimeFormat('en-GB', {
    weekday: 'long',
  });
